...
import { Mail, MapPin, KeyRound } from "lucide-react";
import { SiGithub, SiX } from "@icons-pack/react-simple-icons";
import Link from "next/link";
import PgpKeyBlock, { type PgpKeyEntry } from "./pgp-key-block";

export const metadata: Metadata = {
  title: "Contact | DIBSTER Portfolio",
  description: "Get in touch with DIBSTER.",
};

async function loadPgpKeys(): Promise<PgpKeyEntry[]> {
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
        ...
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription>
              Use these public keys to send me encrypted messages or verify signed releases.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {pgpKeys.length === 0 ? (
              <p className="text-sm text-muted-foreground">No PGP keys published yet.</p>
            ) : (
              pgpKeys.map((entry, index) => (
                <div key={entry.id} className="space-y-6">
                  <PgpKeyBlock entry={entry} />
                  {index < pgpKeys.length - 1 ? <div className="border-t" /> : null}
                </div>
              ))
            )}
          </CardContent>
